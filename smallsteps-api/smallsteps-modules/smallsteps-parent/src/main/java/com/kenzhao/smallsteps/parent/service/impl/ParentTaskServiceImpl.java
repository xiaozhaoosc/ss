...
public class ParentTaskServiceImpl implements IParentTaskService {
    ...
    public List<ParentTaskVo> queryList(ParentTaskBo bo) {
        List<ParentTask> list = parentTaskMapper.selectParentTaskList(bo);
    ...
    public ParentTaskVo queryById(Long taskId) {
        ParentTask parentTask = parentTaskMapper.selectParentTaskByTaskId(taskId);
    ...
    public boolean insertByBo(ParentTaskBo bo) {
        ...
        return parentTaskMapper.insertParentTask(parentTask) > 0;
    }
    ...
    public boolean deleteWithValidById(Long taskId, boolean isValid) {
        return parentTaskMapper.deleteParentTaskByTaskId(taskId) > 0;
    }

    @Override
    public boolean deleteWithValidByIds(List<Long> taskIds, boolean isValid) {
        Long[] ids = taskIds.toArray(new Long[0]);
        return parentTaskMapper.deleteParentTaskByTaskIds(ids) > 0;
    }
    ...
    private ParentTaskVo toVo(ParentTask parentTask) {
        ...
        // 设置状态名称
        vo.setStatusName(parentTask.getStatus() == 1 ? "已完成" : "未完成");
