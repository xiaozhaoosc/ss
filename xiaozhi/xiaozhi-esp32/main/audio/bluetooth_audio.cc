    ...
    switch (event) {
        case ESP_A2D_CONNECTION_STATE_EVT:
            if (param->conn_stat.state == ESP_A2D_CONNECTION_STATE_CONNECTED) {
                ...
                connected_ = true;
                conn_hdl_ = param->conn_stat.conn_hdl;
...
int BluetoothAudio::WriteAudioData(const uint8_t* data, int len) {
    ...
    esp_a2d_audio_buff_t *audio_buf = esp_a2d_audio_buff_alloc(len);
    if (!audio_buf) return 0;
    
    memcpy(audio_buf->data, data, len);
    audio_buf->data_len = len;
    audio_buf->number_frame = 1;    
    
    esp_err_t err = esp_a2d_source_audio_data_send(conn_hdl_, audio_buf);
    if (err == ESP_OK) {
        return len;
    } else {
...
void BluetoothAudio::SetDiscoveryCallback(BluetoothDiscoveryCallback callback) {
