    ...
    while (true) {
        ...
        audio_queue_cv_.wait(lock, [this]() {
            return service_stopped_ ||
                ...
                (!audio_encode_queue_.empty() && audio_offline_queue_.size() < MAX_SEND_PACKETS_IN_QUEUE) ||
        ...
        if (!audio_encode_queue_.empty() && audio_send_queue_.size() < MAX_SEND_PACKETS_IN_QUEUE) {
            ...
            } else if (task->type == kAudioTaskTypeEncodeToOfflineQueue) {
                std::lock_guard<std::mutex> lock(audio_queue_mutex_);
                audio_offline_queue_.push_back(std::move(packet));
            }
...
void AudioService::PushTaskToEncodeQueue(AudioTaskType type, std::vector<int16_t>&& pcm) {
    ...
    if (type == kAudioTaskTypeEncodeToOfflineQueue) {
        task->timestamp = esp_timer_get_time() / 1000; // Use local time as timestamp
    } else if (type == kAudioTaskTypeEncodeToSendQueue && !timestamp_queue_.empty()) {
...
std::unique_ptr<AudioStreamPacket> AudioService::PopPacketFromOfflineQueue() {
    std::lock_guard<std::mutex> lock(audio_queue_mutex_);
    if (audio_offline_queue_.empty()) {
        return nullptr;
    }
    auto packet = std::move(audio_offline_queue_.front());
    audio_offline_queue_.pop_front();
    audio_queue_cv_.notify_all();
    return packet;
}
...
void AudioService::SetCodec(AudioCodec* codec) {
    std::lock_guard<std::mutex> lock(audio_queue_mutex_);
    codec_ = codec;
    ...
    // 刷新解码器重采样
    if (opus_decoder_ && opus_decoder_->sample_rate() != codec_->output_sample_rate()) {
        output_resampler_.Configure(opus_decoder_->sample_rate(), codec_->output_sample_rate());
    }
    ESP_LOGI(TAG, "Codec switched to %p", codec_);
}
