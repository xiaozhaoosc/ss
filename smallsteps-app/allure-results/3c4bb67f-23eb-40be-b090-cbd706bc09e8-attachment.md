...
- Name: spec\parent-emotion-deep.spec.ts >> 情绪相关页面深度测试 >> 情绪急救包页面加载测试
- Location: tests\e2e\spec\parent-emotion-deep.spec.ts:126:7
...
Locator: locator('.section-title, .page-title, view').first()
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('.section-title, .page-title, view').first()
