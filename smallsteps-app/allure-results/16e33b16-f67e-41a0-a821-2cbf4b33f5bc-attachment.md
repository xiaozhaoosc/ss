...
- Name: pages.spec.ts >> 儿童成就页面功能测试 >> 返回按钮应该存在并可点击
- Location: tests\e2e\pages.spec.ts:44:7
...
```ts
  ...
  14 |     await page.goto('http://localhost:9090');
  15 |     
  16 |     // 验证页面标题或基本元素
  ...
  21 |     await page.goto('http://localhost:9090');
  22 |     await expect(page).toHaveTitle(/SmallSteps/);
  23 |   });
  24 | 
  25 |   test('情绪急救包页面应该能正常加载', async ({ page }) => {
  26 |     await page.goto('http://localhost:9090');
  ...
  37 |     await page.goto('http://localhost:9090');
  38 |     
  39 |     // 验证应用是否正常启动
...
> 45 |     await page.goto('http://localhost:9090');
     |                ^ Error: page.goto: Could not connect to server
  46 |     
  47 |     // 验证应用是否正常启动
  ...
  57 |     await page.goto('http://localhost:9090');
  58 |     
  59 |     // 验证应用是否正常启动
  ...
  65 |     await page.goto('http://localhost:9090');
  66 |     
  67 |     // 验证应用是否正常启动
  ...
  77 |     await page.goto('http://localhost:9090');
  78 |     
  79 |     // 验证应用是否正常启动
  ...
  85 |     await page.goto('http://localhost:9090');
  86 |     
  87 |     // 验证应用是否正常启动
