...
Call log:
  - waiting for locator('.tabbar-item').filter({ hasText: '任务' })
...
```ts
  ...
  11 |     this.page = page;
  12 |     this.homeTab = page.locator('.tabbar-item').filter({ hasText: '首页' });
  13 |     this.taskTab = page.locator('.tabbar-item').filter({ hasText: '任务' });
  14 |     this.insightsTab = page.locator('.tabbar-item').filter({ hasText: '洞察' });
  15 |     this.profileTab = page.locator('.tabbar-item').filter({ hasText: '我的' });
  16 |   }
  17 | 
  18 |   async goToHome() {
