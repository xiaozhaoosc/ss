...
- Name: spec\parent-insights-deep.spec.ts >> 洞察菜单深度测试 >> 洞察页面基本元素验证
- Location: tests\e2e\spec\parent-insights-deep.spec.ts:22:7
...
Expected: visible
Error: strict mode violation: getByText('首页') resolved to 2 elements:
    1) <span>首页</span> aka locator('uni-page-body').getByText('首页')
    2) <div class="uni-tabbar__label">首页</div> aka locator('uni-tabbar').getByText('首页')
...
```ts
  ...
  28  | 
  29  |     // 验证底部导航栏
> 30  |     await expect(page.getByText('首页')).toBeVisible();
      |                                        ^ Error: expect(locator).toBeVisible() failed
  31  |     await expect(page.getByText('任务')).toBeVisible();
  32  |     await expect(page.getByText('洞察')).toBeVisible();
  33  |     await expect(page.getByText('我的')).toBeVisible();
  34  |   });
  35  | 
