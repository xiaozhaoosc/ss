...
test.describe('SATS Full-Cycle Workflow: Device to Parent UI', () => {
  ...
  test('should complete the full cycle from device trigger to parent UI check', async ({ page, request }) => {
    // Step 1: 设备触发
    const deviceResponse = await request.post('http://localhost:8098/ssapi/api/device/activation', {
    ...
    // Step 2: 儿童完成任务
    const taskCompleteResponse = await request.post('http://localhost:8098/ssapi/child/task/complete', {
      params: { taskId: 1, childId: childId }
    });
    console.log('Step 2: Child task completed, status:', taskCompleteResponse.status());

    // Step 3: 系统自动发星星
    const starResponse = await request.get(`http://localhost:8098/ssapi/child/achievement/stars/${childId}`);
    if (starResponse.ok()) {
