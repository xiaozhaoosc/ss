...
| **自动化测试本地化改造** | 将 `autotests-demo-local` 中所有 Playwright 测试的 baseURL、远程 API 网关地址、服务调用接口以及 VNC 访问指示等全部从 staging IP (`10.8.0.1`, `10.8.0.3`, `192.168.50.160`) 一键无缝重构为 `localhost`/`127.0.0.1` 本地环回地址，并完美通过 H5 移动端 (81项用例) 与管理后台 UI (19项用例) 的全量语法与配置加载自检。 |
