...
flowchart TB
    ...
    %% ===== System =====
    subgraph SYS["小步 (Small Steps) 辅助系统"]
...
sequenceDiagram
    ...
    participant AI as LLM模块

    Parent->>System: 创建任务
    System->>Child: 下发任务

    Child->>System: 专注打卡/完成任务
    System->>System: 更新行为数据

    System->>Parent: 更新数据看板
...
sequenceDiagram
    ...
    participant Child as 儿童端

    Parent->>Frontend: 创建任务
    Frontend->>Backend: 提交任务配置
    Backend->>DB: 保存任务
    DB-->>Backend: 保存成功

    Backend-->>Frontend: 返回成功
    Backend->>Child: 推送新任务通知

    Child->>Frontend: 查看待办任务
    Frontend->>Backend: 查询任务列表
    Backend->>DB: 获取任务
    DB-->>Backend: 返回数据
